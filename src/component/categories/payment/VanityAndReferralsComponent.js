...
export default function VanityAndReferralsComponent(props) {
  ...
  const textToShow = `https://fomo-shot-9yag.vercel.app/${
    props.signerAddress ? shortenAddress(props.signerAddress) : undefined
  }`;
  const textToCopy = `https://fomo-shot-9yag.vercel.app/${props.signerAddress}`;
  ...
  return (
    <>
      <div className="bg-[#212529] w-[46vw] sm:w-full font-fomofont p-4 rounded-b-2xl rounded-r-2xl">
        <p className="text-base my-1 font-light text-center font-fomofont">
          Advise others to invest in this exit scam and we'll reward you 10% of
          everything they lose. In SOS
        </p>

        <div className="flex flex-col justify-center items-center my-3 text-base font-light font-fomofont">
          <h4 className=" text-xl font-normal font-fomofont">
            Wallet Referral
          </h4>
          <p>{textToShow}</p>
          <button
            ref={copyButtonRef}
            onClick={() => copyToClipboard(props.signerAddress)}
            className="px-3 my-1 font-fomopink font-normal text-base flex items-center justify-center border hover:text-white hover:bg-[#f000f0] rounded-md py-1.5 border-[#f000f0]"
          >
